...
eventsController.get(`/events`, (_req, res) => {
...
eventsController.get(`/careRecipient/:care_recipient_id/events`, (_req, res) => {
  ...
  let sql = `SELECT DISTINCT event_type from events WHERE care_recipient_id = '${care_recipient_id}'`;
  let query = db.query(sql, (err: any, results: any) => {
    if (err) {
      throw err;
    }
...
eventsController.get(`/careRecipient/:care_recipient_id/events/:event_type`, (_req, res) => {
    ...
    let care_recipient_id = _req.params.care_recipient_id;
    let sql = `SELECT * from events WHERE event_type = '${event_type}' AND care_recipient_id = '${care_recipient_id}' ORDER BY timestamp`;
    let query = db.query(sql, (err: any, results: any) => {
      if (err) {
        throw err;
      }
